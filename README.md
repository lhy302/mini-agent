...
| 可调用工具 | 全部面向 AI 自主调用：`pwsh`（命令执行）、`str_replace_editor`（文件查看编辑）、`mouse_control`（鼠标自主操作）、`read_image`（自主读图送入多模态分析）、`take_screenshot`（自主截屏分析 GUI） |
